...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const publishedCount = courses.reduce((s, c) => s + lessonCount(c), 0);
    ...
    <div className="mx-auto max-w-[1100px] px-4 py-6">
      ...
      <header className="animate-fade-up mt-4 flex flex-wrap items-center justify-between gap-3 overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary-soft to-surface p-6 shadow-[var(--shadow-sm)]">
        <div>
          ...
          <p className="mt-1 text-[15px] text-text-muted">Create and manage lessons, notes and quizzes.</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { label: "Published lessons", value: publishedCount },
      ...
      <section className="mt-8">
        <h2 className="mb-3 border-b border-border pb-2 text-lg font-bold text-text">My Lessons (drafts)</h2>
        {drafts.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border-strong bg-surface p-8 text-center">
            <p className="text-sm text-text-muted">No drafts yet. Create your first lesson.</p>
      ...
      <section className="mt-8">
        <h2 className="mb-3 border-b border-border pb-2 text-lg font-bold text-text">Published Subjects</h2>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {courses.map((c) => (
            ...
            >
              ...
              <span className="ml-2 shrink-0 text-xs text-text-faint">{lessonCount(c)} lessons</span>
            </Link>
          ))}
        </div>
      </section>
